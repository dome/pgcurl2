...
Datum pgcurl_post(PG_FUNCTION_ARGS)
{
  return curl_do_actual_work(PGCURL_POST, fcinfo);
}
...
Datum curl_do_actual_work(int method_type, PG_FUNCTION_ARGS)
{
  ...
  CURLcode res;
  StringInfoData buf;
  text *url_text = PG_GETARG_TEXT_P(0), *fields_text;
  char *url, *fields;
  ...
  if(curl) {
    ...
    if (method_type == PGCURL_POST) {
      fields_text = PG_GETARG_TEXT_P(1);
      fields = DatumGetCString(DirectFunctionCall1(textout, PointerGetDatum(fields_text)));
